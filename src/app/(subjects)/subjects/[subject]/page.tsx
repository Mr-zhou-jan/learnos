...
const CHAPTERS: Record<string, { name: string; desc: string; progress: number }[]> = {
  "高等数学": ["函数与极限","导数与微分","中值定理与导数应用","不定积分","定积分","微分方程"].map(n=>({name:n,desc:"",progress:0})),
  "大学物理": ["质点运动学","牛顿定律","动量与能量","刚体转动"].map(n=>({name:n,desc:"",progress:0})),
  "工程力学": ["静力学基础","平面力系","材料力学基础"].map(n=>({name:n,desc:"",progress:0})),
};

function getChapters(sub: string) {
  for (const [k, v] of Object.entries(CHAPTERS)) if (sub.includes(k)) return [...v];
      ...
      <div className="space-y-2">
        {chapters.map((ch, i) => (
          <button key={ch.name} onClick={() => router.push(`/subjects/${encodeURIComponent(decoded)}/${encodeURIComponent(ch.name)}`)}
            ...
            <div className="flex-1"><p className="font-semibold text-sm">{ch.name}</p></div>
