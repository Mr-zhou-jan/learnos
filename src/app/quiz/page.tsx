...
function QuizContent() {
  ...
  const finish = useCallback(()=>{const ns:Record<string,any>={};answers.forEach(a=>{if(!ns[a.nodeId])ns[a.nodeId]={correct:0,total:0,title:a.nodeTitle};ns[a.nodeId].total++;if(a.correct)ns[a.nodeId].correct++;});localStorage.setItem("learnos_quiz_results",JSON.stringify({answers,totalCorrect:answers.filter(a=>a.correct).length,totalQuestions:answers.length,nodeScores:Object.entries(ns).map(([id,d])=>({nodeId:id,title:d.title,score:Math.round((d.correct/d.total)*100),correct:d.correct,total:d.total})),completedAt:new Date().toISOString()}));localStorage.setItem("learnos_diagnosed","true");router.push("/plan?name="+encodeURIComponent(names[0]||""));},[answers,router,names]);

  if(loading)return<div className="min-h-screen bg-page flex items-center justify-center"><div className="w-full max-w-lg"><div className="card-glass p-8 space-y-5 animate-pulse"><div className="skeleton h-6 w-32 mx-auto rounded-full"/><div className="skeleton h-2 w-full rounded-full"/><div className="space-y-3 mt-6"><div className="skeleton h-14 w-full rounded-2xl"/><div className="skeleton h-14 w-full rounded-2xl"/><div className="skeleton h-14 w-full rounded-2xl"/><div className="skeleton h-14 w-full rounded-2xl"/></div></div><p className="text-center text-zinc-400 text-sm mt-5">AI 正在生成诊断题目…</p></div></div>;
  if(submitted){const c=answers.filter(a=>a.correct).length;const s=answers.length>0?Math.round((c/answers.length)*100):0;const bd:Record<string,{correct:number;total:number}>={};answers.forEach(a=>{if(!bd[a.nodeTitle])bd[a.nodeTitle]={correct:0,total:0};bd[a.nodeTitle].total++;if(a.correct)bd[a.nodeTitle].correct++;});
    return(<div className="min-h-screen bg-page flex items-center justify-center p-6"><div className="w-full max-w-md card-glass p-8 animate-bounce-in"><div className="text-center"><div className="w-16 h-16 mx-auto rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mb-4 shadow-lg shadow-purple-200"><GraduationCap className="w-8 h-8 text-white"/></div><h2 className="text-2xl font-bold">诊断完成</h2><div className="mt-4"><span className={cn("text-6xl font-extrabold",s>=80?"text-emerald-600":s>=50?"text-amber-600":"text-red-600")}>{s}</span><span className="text-zinc-300 text-2xl ml-1">/100</span></div><p className="text-zinc-500 mt-2">答对 {c}/{answers.length} 题</p></div><div className="mt-6 space-y-2 max-h-64 overflow-y-auto">{Object.entries(bd).slice(0,8).map(([t,d])=>{const pct = Math.round((d.correct/d.total)*100);return(<div key={t} className="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-zinc-50 transition-colors"><span className="text-sm truncate flex-1 font-medium">{t}</span><div className="w-16 h-1.5 bg-zinc-100 rounded-full overflow-hidden shrink-0"><div className={cn("h-full rounded-full",pct===100?"bg-emerald-500":pct===0?"bg-red-500":"bg-amber-500")} style={{width:pct+"%"}}/></div><span className={cn("text-xs font-bold w-10 text-right",pct===100?"text-emerald-600":pct===0?"text-red-600":"text-amber-600")}>{d.correct}/{d.total}</span></div>);})}</div><button onClick={finish} className="btn-primary w-full mt-6 text-base"><Sparkles className="w-5 h-5"/>查看专属计划<ArrowRight className="w-5 h-5"/></button></div></div>);}
