...
function ErrorBookInner() {
  ...
  const refresh = () => { setGrouped(getErrorBookByModule()); setStubborn(getStubbornVocabWords()); };
  useEffect(() => { refresh(); }, []);
  useEffect(() => {
  ...
  const list = tab === "stubborn" ? [] : tab === "all" ? all : (grouped[tab] || []);
